...
model and Sonnet agents for bounded work. It is not a mandatory pipeline:
...
```sh
claude --model fable   # or pin the exact model: claude --model claude-fable-5
```
...
definition. The three definitions in `.claude/agents/` set their workers to
the `sonnet` alias — the current Sonnet tier, deliberately not a fixed
version — and choose effort and tools for each role:
